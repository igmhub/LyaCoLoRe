...
#Ideally want to define this. However need to work out how the multiprocessing will deal with it
#Want each worker process to run 'get_objects', adding the objects to shared memory
#Is this possible? Or will it have to be done in the same way as is currently done? (using 'results' and then merging)
#Maybe can run the multiprocessing internally?
class colore_simulation:

    def __init__(self,box_size,N_nodes,location,filenames):
    ...
    def get_data(self):
        ...
        for filename in self.filenames:
            ...
            #get the Z, RA, DEC, MOCKID for each object

        return

    def make_pixel_skewers_object(self,pixel):

        pixel_CATALOG = self.CATALOG[self.CATALOG[PIXNUM==pixel]]
        pixel_skewers = colore_skewers(pixel_CATALOG,COSMOLOGY)

        return pixel_skewers

class colore_skewers:
    #Initialisation function.
    def __init__(self,CATALOG,COSMOLOGY):

        self.N_qso = CATALOG.shape[0]

        self.TYPE = CATALOG['TYPE']
        self.RA = CATALOG['RA']
        self.DEC = CATALOG['DEC']
        self.Z_QSO = CATALOG['Z_QSO']
        self.DZ_RSD = CATALOG['DZ_RSD']
        self.MOCKID = CATALOG['MOCKID']
        self.PLATE = CATALOG['PLATE']
        self.MJD = CATALOG['MJD']
        self.FIBER = CATALOG['FIBER']

        self.R = COSMOLOGY['R']
        self.Z = COSMOLOGY['Z']
        self.D = COSMOLOGY['D']
        self.V = COSMOLOGY['V']
        self.LOGLAM_MAP = COSMOLOGY['LOGLAM_MAP']

        return


    def get_gaussian_skewers(self):

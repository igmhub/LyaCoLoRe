...
#Assumes that the object already has tau calculated and RSDs applied.
def get_bias_nu(data,betas,z_values,d=0.001,z_width=0.2,z_r0=2.5):

    #Find the pivot distance, and calculate extra velocity.
    r0 = np.interp(z_r0,data.Z,data.R)
    dv = (data.R - r0) * d

    #Add small extra grad to velocity skewers.
    grad_increase = copy.deepcopy(data)
    grad_increase.VEL_rows += dv
    grad_increase.add_all_RSDs(thermal=include_thermal_effects)

    #Subtract small extra grad to velocity skewers.
    grad_decrease = copy.deepcopy(data)
    grad_decrease.VEL_rows -= dv
    grad_decrease.add_all_RSDs(thermal=include_thermal_effects)

...
To do:
 ...
 - work out how to take picca as input or colore as output
 - can input format be detected? yes as CoLoRe outputs a blank primary hdu whereas picca takes in deltas in primary hdus
    - currently assume CoLoRe input. picca input would be useful for when test data sets need to be made by cutting objects from pre-existing per-pixel picca file_suffices
...
def split_pixels(original_location,original_filename_structure,file_numbers,input_format,N_side,z_min,save_location,new_filename_structure,output_format):
    ...
    for pixel in pixel_list:
        ...
        for node in relevant_nodes:
            ...
            filename = (original_location + '/' + original_filename_structure.format(int(node)))

            #Extract the data from the file.
            working = get_data(filename,input_format,relevant_THING_IDs,z_min)
...
#Function to extract data from a fits file.
def get_data(filename,input_format,THING_IDs,z_min):

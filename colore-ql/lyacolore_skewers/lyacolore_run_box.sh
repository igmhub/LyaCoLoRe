...
CONFIG_FILE="/path/to/config_v9.0.ini"

CONDA_ENVIRONMENT=lyacolore # Conda environment to use

# Set where your CoLoRe output is located, and where you would like your
# LyaCoLoRe output to be located.
COLORE_OUT_LOC="/path/to/colore/box/results"
LYACOLORE_OUT_LOC="/output/path"
...
for NODE in \`seq $NNODES\` ; do
    ...
    command="srun -N 1 -n 1 -c 256 ${LYACOLORE_PATH}/scripts/make_transmission.py -c ${CONFIG_FILE} -i ${COLORE_OUT_LOC} -o ${LYACOLORE_OUT_LOC} --nproc ${NCORES} --pixels \${NODE_PIXELS}"

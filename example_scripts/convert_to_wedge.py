...
R_min = 0.
R_max = 160.
mu_bins = np,array([(0.0,0.1),(0.45,0.55),(0.9,1.0)])
...
mu_original = R_original/RT_bins

R_bin_numbers = np.array(list(range(N_bins)))
binned_R_numbers = np.digitize(R_original,R_edges) - 1
binned_R_values = (binned_R_numbers + 0.5) * R_bin_size

binned_data = np.zeros((mu_bins.shape[0],R_bins.shape[0]))
...
for i,mu_bin in enumerate(mu_bins):
    ...
    for j,R_bin in enumerate(R_bins):

        relevant = mu_relevant * (binned_R == R_bin)

        binned_data[i,j] = np.average(DA_mean[relevant],weights=WE_sum[relevant])

    plt.plot(binned_R_values,binned_data[i,:],label=str(mu_bin))

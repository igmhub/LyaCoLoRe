...
def generate_rnd(factor=3, out_path=None):
    ...
    data = fitsio.read('/global/projecta/projectdirs/desi/mocks/lya_forest/london/v4.0.0/DLA.fits.gz')
    zvec = data['ZDLA']
    ntot = int(len(data)*factor)
    z_rnd = np.random.choice(zvec,size=ntot)+0.0025*np.random.normal(size=ntot)
    ra_rnd = 360*np.random.random(size=ntot)
    dec_rnd = np.degrees(np.arcsin(-1+2*np.random.random(size=ntot)))
    if out_path is not None:
        tab_out = astropy.table.Table([ra_rnd,dec_rnd,z_rnd],names=('RA','DEC','Z'))
        tab_out.write(out_path,overwrite=True)
    return None
# Execute
generate_rnd(factor=10,out_path='/global/projecta/projectdirs/desi/mocks/lya_forest/london/v4.0.0/DLA_randoms.fits.gz')
